...
export const TokenCardContainer = (props) => {
  const { token, setToken, setIsTokenPage, mode, service, isConnected } = props;
  ...
  useEffect(() => {
    if (data) {
      dispatch(getTokensDefiById(data));
    }
  ...
  async function getConnectedChain() {
    setToken(token);
    dispatch(updateChain(token));
    dispatch(getTokensDefiById(token?.chainId));
  ...
  async function getNotConnectedChain() {
    setToken(token);
    dispatch(updateChain(token));
    dispatch(getTokensDefiById(token?.chainId));
    localStorage.setItem('chainSwitch', JSON.stringify(true));
